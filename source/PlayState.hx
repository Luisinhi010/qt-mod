...
class PlayState extends MusicBeatState
{
	...
	public var qtTVanimations:Array<String> = [
		"idle", "alert", "instructions", "gl", "watch", "eye", "error", "404", "heart", "drop", "sus"
	];

	var qtTVstate(default, set):Int = 0;

	function set_qtTVstate(value:Int):Int
	{
		/*
		 *	0 = static
		 *	1 = alert
		...
		if (qt_tv01 != null)
			qt_tv01.animation.play((value == 420 || value == 69) ? "sus" : qtTVanimations[value]);

		return qtTVstate = value;
	}
	...
	override public function create()
	{
		...
		// Alert!
		kb_attack_alert = new KbAttackAlert();
	...
	public function kbATTACK_ALERT(alertType:Int = 1, playSound:Bool = true):Void
	{
		...
		switch (alertType)
		{
			case 2:
				alpha = 0.55;
				animName = 'alertDOUBLE';
				soundName = 'hazard/alertDouble';
			case 3:
				alpha = 0.5875;
				animName = 'alertTRIPLE';
				soundName = 'hazard/alertTriple';
			case 4:
				alpha = 0.6;
				animName = 'alertQUAD';
				soundName = 'hazard/alertQuadruple';
			default:
				alpha = 0.49;
				animName = 'alert';
				soundName = 'hazard/alert';
		}
		if (playSound)
			FlxG.sound.play(Paths.sound(soundName), 1);

		if (ClientPrefs.noteOffset <= 0)
			kbATTACK_ALERT_PART2(alpha, animName);
		else
		{
	...
	function kbATTACK_ALERT_PART2(newAlpha:Float, animationToPlay:String):Void
	{
		...
		if (ClientPrefs.flashing && !ClientPrefs.lowQuality && hazardOverlayShit != null)
	...
	public function KBPINCER_PREPARE(laneID:Int, goAway:Bool):Void
	{
		...
		if (!goAway)
		{
			...
			// add(pincer);
			insert(members.indexOf(timeTxt), pincer);
	...
	function TerminationIntroShit(state:Int, playerShit:Bool):Void
	{
		...
		var spr:StrumNote = null;
		if (playerShit)
			spr = playerStrums.members[state];
		else
			spr = opponentStrums.members[state];
		if (spr != null)
		{
			spr.y += (ClientPrefs.downScroll ? 100 : -100);
			FlxTween.tween(spr, {y: hazardModChartDefaultStrumY[state], alpha: (!playerShit && forceMiddleScroll) ? 0.35 : 1}, 1.22, {
	...
	{
		var strums = state > 3 ? playerStrums : opponentStrums;
		var targetState = state > 3 ? state - 4 : state;
		FlxTween.tween(strums.members[targetState], {alpha: 0}, 1.1, {ease: FlxEase.sineInOut});
	}

	public function KBPINCER_GRAB(laneID:Int):Void
		if (laneID > 6)
			return trace("Invalid LaneID for pincerGRAB");
		else
			pincers[(laneID - 1) % 4].loadGraphic(Paths.image('hazard/qt-port/pincer-close'), false);
