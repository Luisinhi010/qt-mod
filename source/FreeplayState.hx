...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		addSong("Termination", 0, 'kb', FlxColor.fromRGB(69, 69, 69));
	...
	override function update(elapsed:Float)
	{
		...
		if (songs[curSelected].songName != "")
		{
			...
			var formattedSong:String = Highscore.formatSong(songLowercase.toLowerCase(), curDifficulty);
			PlayState.SONG = Song.loadFromJson(formattedSong, songLowercase.toLowerCase());

			if (space && songs[curSelected].songName != "Interlope")
			{
	...
	function prepareSongAssets():Void
	{
		...
		{
			vocals = loadVocals(PlayState.SONG.song);
		}
		else
		{
			vocals = new FlxSound();
	...
	function loadVocals(songName:String):FlxSound
	{
		return ClientPrefs.qtOldVocals ? new FlxSound().loadEmbedded(Paths.voicesCLASSIC(songName)) : new FlxSound().loadEmbedded(Paths.voices(songName));
	}

	function switchToAppropriateState():Void
	{
